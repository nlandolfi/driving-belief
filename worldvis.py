...
class Moment(object):
    def __init__(self, world, t, version, center=None):
        ...
        if center is None:
            ...
            self.human = world.robots[center].x[t]
            self.robots = [robot.x[t] for robot in world.robots if robot!=world.robots[center]]
            self.robots.append(world.human[version].x[t])
...
class World(object):
    def __init__(self, T, dyn, lanes=[], fences=[], roads=[], ncars=1):
        ...
        self.target = vector(2)
        self.target_importance = vector(1)
    @property
    def df(self):
    ...
    def optimizer(self):
        ...
        for human in self.human.values():
            variables += human.u
        return Maximizer(obj, variables)
    ...
    def load(self, snapshot):
        load(self, snapshot)
    def moment(self, *args, **vargs):
        return Moment(self, *args, **vargs)
    def randomize(self):
        for x, B in self.bounds.iteritems():
            x.set_value(np.array([np.random.uniform(a, b) for (a, b) in B]))
    @cached_property
    def env_optimizer(self):
        dist = tt.sqrt(sum((self.target[0]-x[0])**2+(self.target[1]-x[1])**2 for x in self.robots[0].x))
        return Maximizer(
            tt.dot(self.w, self.fvector('A', 0))-self.target_importance[0]*dist,
            self.robots[0].u
        )
    def genenv(self):
        self.randomize()
        self.target.set_value([np.random.uniform(-0.2, 0.2), .9])
        self.target_importance.set_value([np.random.uniform(0., 20.)])
        self.env_optimizer.maximize(bounds = self.bounds)
